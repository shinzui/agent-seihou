...
4. Read the child ExecPlan file. Follow `agents/skills/{{exec-plan.skill.name}}/SKILL.md` (Mode: implement): verify milestone outcomes, update the child plan on milestone completion, material change, or handoff, and commit at meaningful working boundaries. Every commit must include both `MasterPlan:` and `ExecPlan:` git trailers.
