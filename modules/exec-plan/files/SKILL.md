...
```bash
bun agents/skills/{{skill.name}}/record-provenance.ts review \
  --plan <plan-path> --model <your-model-id> [--harness <name>] \
  --verdict <approved|changes-requested|comments> --note "<one line>"
```

```bash
bun agents/skills/{{skill.name}}/record-provenance.ts revision \
  --plan <plan-path> --model <your-model-id> [--harness <name>] \
...
3. Run the init script to create the file with frontmatter and skeleton:
    ...
    ```bash
    bun agents/skills/{{skill.name}}/init-plan.ts --title "<short, action-oriented title>" --model <your-model-id> [--harness <name>] [--intention <id>] [--master-plan <path>]
    ```
...
6. Record the review in the plan's frontmatter:
    ...
    ```bash
    bun agents/skills/{{skill.name}}/record-provenance.ts review --plan <plan-path> --model <your-model-id> [--harness <name>] --verdict <approved|changes-requested|comments> --note "<one line>"
    ```
