...
title: docs-sync
description: Discover a project's local and sibling documentation/agent-context surfaces,
  then audit and repair stale surfaces against authoritative code using a baseline-SHA
  ledger
resource: seihou://agent-seihou/blueprints/docs-sync
tags:
- documentation
- docs
- cli
...
Discover a project's local and sibling documentation/agent-context surfaces, then audit and repair stale surfaces against authoritative code using a baseline-SHA ledger
...
- `sync.scope` — text, required, default `all`. Surfaces to audit. Use 'all' for every stale discovered surface, or a comma-separated list of surface keys from docs/docs-sync.json (for example 'readme,cli-help,agent-context'). Discovery always runs, but baselines advance only for audited surfaces.
- `external.policy` — text, required, default `discover`, matching `(discover|local-only)`. Whether to discover and sync confirmed sibling kit/docs repositories ('discover') or restrict all edits to the current repository ('local-only').

## Reference files

- `capability-discovery.md` - Evidence rules for discovering local documentation, CLI help transport and reachability, embedded agent-assist context, and sibling kit/docs repositories without relying on project-specific names or language conventions.
- `ledger-reference.md` - Portable docs/docs-sync.json shape, baseline invariants, staleness rules, and examples for local and cross-repository surfaces.
